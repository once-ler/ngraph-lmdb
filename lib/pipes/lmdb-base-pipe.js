 ...
 *
 * @param {VertexLmdbIterator||EdgeLmdbIterator} sourcePipe should be an object which implement `moveNext()` and
 *   `current()` methods
 */
...
LmdbBasePipe.prototype.pipe = function (destinationPipe) {
  ...
  //console.log('lmdb-base-pipe:pipe->'+JSON.stringify(this))
...
LmdbBasePipe.prototype.getCurrentPath = function (callback) {
  ...
  /*
  var path = [];
  var context = this._sourcePipe;
  do {
    path.push(context._currentEdgesIterator ? context._currentEdgesIterator._current : context._current);
    context = context._sourcePipe;
  }
  while (context);
  ...
  */
  ...
  else if (this._currentVerticesIterator){
    //pathToHere.push(this._currentVerticesIterator._current);
    console.log(this)
    /*
    if (this._fromNode && this._toNode){
      pathToHere.push([this._fromNode, this._toNode]);
    }
    if (this._fromNode && !this._toNode){
      pathToHere.push(this._fromNode); 
    }
    if (!this._fromNode && this._toNode){
      pathToHere.push(this._toNode); 
    }
    */ 
  } 
  ...
  //pathToHere.push(this._currentEdgesIterator ? this._currentEdgesIterator._current : this._current);
  return pathToHere;
...
function getPathToHere(basePipe) {
  ...
  //throw new Error('getPathToHere() is not implemented');
