  ...
  if (filter){
    ...
    if (typeof filter == 'string'){
      ...
      if (db == 'vertex'){
        ...
        var inFilter = filters[1] + ':' + filters[0];
        filter = db == 'in' ? inFilter : filter;
        //var edgeLabel = filters[1];
        /**
          * if mode == 'both', filter is nodeId:label OR label:nodeId
          */
        var patt = new RegExp(db != 'both' ? filter : '('+filter + '|' + inFilter + ')', 'i');
        this.filterArr = [];
        var self = this;
    ...
    if (typeof filter == 'object') {
      ...
      this.filterArr = [];
      var self = this;
      //TODO: Need to test searchForDocuments for edges
      for(var k in filter){
        var results;
        if (db == 'vertex') {        
          results = self.indexVertexEngine.searchForDocuments(k+':'+filter[k]);
        }
        else {
          results = self.indexEdgeEngine.searchForDocuments(k+':'+filter[k]);
        }
