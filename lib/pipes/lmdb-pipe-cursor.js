...
function PipeCursor(db, filter) {
  ...
  if (filter){
    ...
    if (typeof filter == 'object') {
      ...
      var self = this;
      for(var k in filter){
        var results = self.indexVertexEngine.searchForDocuments(k+':'+filter[k].toLowerCase());
        for(var j in results){
          self.filter[j] = true;
        }
      }

      for(var l in this.filter){
        self.filterArr.push(self.filter[l]);
      }
...
PipeCursor.prototype.goToKey = function(key){
  return this.cursor.goToKey(key);
}
